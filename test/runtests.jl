...
@testset "Nodiv.jl" begin
    ...
    @testset "occupied mask is deterministic and excludes empty cells" begin
        Random.seed!(7)
        sims = Float64.(rand(0:4, 51, 8)); sims[1, 1:3] .= 10.0
        empty = hcat(sims, zeros(51))            # append a parent-absent (all-zero) cell
    ...
    @testset "cached-SOS helpers match the sims-level functions" begin
        Random.seed!(1)
        sims = Float64.(rand(0:5, 81, 30)); sims[1, 1:6] .= 9.0
        @test isapprox(gnd_rms(calculate_SOS(sims)),     calculate_GND_rms(sims);     rtol = 1e-9)
        @test isapprox(gnd_spatial(calculate_SOS(sims)), calculate_GND_spatial(sims); rtol = 1e-9)
    end

    @testset "swap is the only null (no method argument)" begin
        # simulate_descendants and the entry points no longer accept `method`
        @test !(:method in Base.kwarg_decl(only(methods(simulate_descendants))))
        @test !(:method in Base.kwarg_decl(only(methods(node_metrics))))
    end
