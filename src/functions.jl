...
function _sos_distances(sosmat; minoverlap::Integer = 3, method::Symbol = :pearson,
                        overlapweight::Bool = false, occupied = nothing)
    corfun = method === :pearson  ? cor :
             method === :spearman ? corspearman :
             error("`method` must be :pearson or :spearman")
    overlapweight && occupied === nothing &&
        error("overlapweight = true needs an `occupied` mask (pass `assemblage`/`tree`, " *
...
_occupied_matrix(assemblage, tree, nodes) =
    reduce(hcat, (richness(get_clade(assemblage, tree, n)) .> 0) for n in nodes)

sos_distances(sosvectors::AbstractVector; kw...) = _sos_distances(reduce(hcat, sosvectors); kw...)

function sos_distances(res::Union{NodeAnalysis, NodeMetrics}, nodes;
                       overlapweight::Bool = false, assemblage = nothing, tree = nothing, kw...)
    occupied = nothing
    if overlapweight
        (assemblage === nothing || tree === nothing) &&
            error("overlapweight = true on a `res` needs `assemblage` and `tree` to derive occupancy")
        occupied = _occupied_matrix(assemblage, tree, nodes)
    end
    _sos_distances(reduce(hcat, res.sos[n] for n in nodes); overlapweight, occupied, kw...)
end
...
function sos_distances(assemblage, tree, nodes; nsims = 200, overlapweight::Bool = false, kw...)
    ...
    _sos_distances(sosmat; overlapweight, occupied, kw...)
end
