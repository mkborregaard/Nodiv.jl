"""
    simulate_descendants(clade, tree, descendant; nsims=200, rng=default_rng())
        -> Matrix{Float64}
...
a difference in where they occur. The draws come from `rng`.
"""
function simulate_descendants(
    clade, tree, descendant; nsims=200, rng::AbstractRNG=default_rng()
)
    return _simulate_descendants(clade, nodespecies(tree, descendant); nsims, rng)
end
...
function _simulate_descendants(
    clade, descsp::Vector{String}; nsims=200, rng::AbstractRNG=default_rng()
)
    ret = zeros(nsims + 1, nsites(clade))
    ret[1, :] = richness(view(clade; species=descsp))  # observed
    rmg = matrixrandomizer(clade, rng)
    for i in 2:(nsims + 1)
